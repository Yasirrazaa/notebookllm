...
# Conditionally-imported types used for annotations only
try:
...
class KernelPool:
    ...
    Thread-safe — all ``_kernels`` and ``_jobs`` access is guarded by ``self._lock``.
    """
    ...
    async def execute_all_cells(
        ...
        for i, cell in enumerate(cells):
            ...
            # Check the returned output for error indicators rather than
            # fragile job-list lookups.
    ...
    def has_kernel(self, session_id: str) -> bool:
        """Check if a kernel exists for the given session."""
        with self._lock:
